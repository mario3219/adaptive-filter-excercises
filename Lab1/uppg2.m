figure('Position',[10 400 350 415]);
figure('Position',[375 400 430 415]);
figure('Position',[820 400 320 415]);

figure(1)
uicontrol(gcf,...
'Style','text',...
'Position',[10 390 220 20],...
'String','KANALUTJAEMNING 2');

uicontrol(gcf,...
'Style','text',...
'Position',[10 360 150 20],...
'String','kanalens graensfr [Hz] =');
Fg=uicontrol(gcf,...
'Style','edit',...
'Position',[165 360 30 20],...
'String','1');
uicontrol(gcf,...
'Style','text',...
'Position',[205 360 60 20],...
'String','ordning =');
ordn=uicontrol(gcf,...
'Style','edit',...
'Position',[270 360 30 20],...
'String','2');
akanal=uicontrol(gcf,...
'Style','pushbutton',...
'Position',[10 325 80 30],...
'String','studera kanal',...
'Value',0,...
'Callback',[...
'F=str2num(get(Fg,''String''));',...
'Fs=str2num(get(Fsa,''String''));',...
'K=str2num(get(ordn,''String''));',...
'[H,Fv]=studa2(K,F,Fs);']);

uicontrol(gcf,...
'Style','text',...
'Position',[100 330 100 20],...
'String','samplingsfrekvens=');
Fsa=uicontrol(gcf,...
'Style','edit',...
'Position',[205 330 30 20],...
'String','5');
kanal=uicontrol(gcf,...
'Style','pushbutton',...
'Position',[245 325 100 30],...
'String','diskretiserad kanal',...
'Value',0,...
'Callback',[...
'F=str2num(get(Fg,''String''));',...
'Fs=str2num(get(Fsa,''String''));',...
'K=str2num(get(ordn,''String''));',...
'[Bz,Az]=studd2(K,F,Fs);']);

uicontrol(gcf,...
'Style','text',...
'Position',[10 290 130 20],...
'String','symbolintervall [sampel] =');
inte=uicontrol(gcf,...
'Style','text',...
'Position',[145 290 30 20],...
'String','5');
uicontrol(gcf,...
'Style','text',...
'Position',[185 290 70 20],...
'String','signalint. =');
sigint=uicontrol(gcf,...
'Style','edit',...
'Position',[260 290 30 20],...
'String','40');
inteval=uicontrol(gcf,...
'Style','slider',...
'Min',1,...
'Max',10,...
'Value',5,...
'Position',[10 265 170 20],...
'Callback',[...
'figure(2);',...
'N=round(get(inteval,''Value''));',...
'set(inte,''String'',num2str(N));']);

uicontrol(gcf,...
'Style','text',...
'Position',[10 240 70 20],...
'String','brusvarians =');
vari=uicontrol(gcf,...
'Style','edit',...
'Position',[85 240 40 20],...
'String','.000');

uicontrol(gcf,...
'Style','text',...
'Position',[10 200 30 20],...
'String','mu=');
adk=uicontrol(gcf,...
'Style','text',...
'Position',[45 200 30 20],...
'String','.5');
adkonst=uicontrol(gcf,...
'Style','slider',...
'Min',0,...
'Max',3,...
'Value',.5,...
'Position',[80 200 170 20],...
'Callback',[...
'figure(2);',...
'mu=get(adkonst,''Value'');',...
'set(adk,''String'',num2str(mu));']);

uicontrol(gcf,...
'Style','text',...
'Position',[10 170 60 20],...
'String','utj.-ordn. =');
adordn=uicontrol(gcf,...
'Style','edit',...
'Position',[75 170 30 20],...
'String','30');

uicontrol(gcf,...
'Style','text',...
'Position',[115 170 70 20],...
'String','uppd.-int. =');
udati=uicontrol(gcf,...
'Style','edit',...
'Position',[190 170 30 20],...
'String','6');

uicontrol(gcf,...
'Style','text',...
'Position',[10 140 50 20],...
'String','foerdr.=');
delej=uicontrol(gcf,...
'Style','edit',...
'Position',[65 140 30 20],...
'String','15');

uicontrol(gcf,...
'Style','pushbutton',...
'Position',[10 100 50 25],...
'String','START',...
'Callback',[...
'qb=length(Bz);',...
'qa=length(Az);',...
'p=str2num(get(adordn,''String''));',...
'br=str2num(get(vari,''String''));',...
'S=str2num(get(udati,''String''));',...
'del=str2num(get(delej,''String''));',...
'M=str2num(get(sigint,''String''));',...
'w=zeros(1,p);',...
'x=zeros(M,1);',...
'y=zeros(M,1);',...
'xh=zeros(M,1);',...
't=1;',...
'set(stop,''Value'',0);',...
'st=0;',...
'while st==0;',...
'  N=round(get(inteval,''Value''));',...
'  mu=get(adkonst,''Value'');',...
'  x=[0; x(1:length(x)-1)];',...
'  if mod(t,N)==0;',...
'    x(1)=2*round(rand)-1;',...
'  end;',...
'  y(2:length(y))=y(1:length(y)-1);',...
'  y(1)=Bz*x(1:qb)-Az(2:qa)/Az(1)*y(2:qa)+sqrt(br)*randn;',...
'  xh(2:M)=xh(1:M-1);',...
'  xh(1)=w*y(1:p);',...
'  d=x(1+del);',...
'  w=w+mu*( d-xh(1) )*rot90(y(1:p));',...
'  if mod(t,S)==0;',...
'    visa2(w,x,y,xh,p);',...
'    pause(.1);',...
'  end;',...
'  t=t+1;',...
'  st=get(stop,''Value'');',...
'end;';]);

stop=uicontrol(gcf,...
'Style','checkbox',...
'Position',[70 100 60 25],...
'Value',0,...
'String','STOPP',...
'Callback',[...
'figure(3);',...
'subplot(222);',...
'set(stop,''Value'',1);';]);

uicontrol(gcf,...
'Style','pushbutton',...
'Position',[140 100 60 25],...
'Value',0,...
'String','invers',...
'Callback',[...
'visai2(Bz,Az);']);

uicontrol(gcf,...
'Style','pushbutton',...
'Position',[210 100 60 25],...
'Value',0,...
'String','Wiener',...
'Callback',[...
'N=round(get(inteval,''Value''));',...
'br=str2num(get(vari,''String''));',...
'visaw2(Bz,Az,N,br);']);

uicontrol(figure(1),...
'Style','pushbutton',...
'String','Avsluta',...
'Position',[275 40 70 30],...
'Callback',[...
'close;',...
'close;',...
'close;']);

